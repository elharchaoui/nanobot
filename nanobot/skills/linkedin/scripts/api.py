...
Usage:
    ...
    python3 api.py post "Your update text here" --image path/to/image.jpg
    python3 api.py post "Your update text here" --video path/to/video.mp4
...
def upload_media(cfg: dict, media_path: str, recipe: str) -> str:
    """Upload a media file to LinkedIn and return the asset URN."""
...
def upload_image(cfg: dict, image_path: str) -> str:
    ...
    return upload_media(cfg, image_path, "urn:li:digitalmediaRecipe:feedshare-image")


def upload_video(cfg: dict, video_path: str) -> str:
    """Upload a video to LinkedIn and return the asset URN."""
    return upload_media(cfg, video_path, "urn:li:digitalmediaRecipe:feedshare-video")


def cmd_post(cfg: dict, text: str, image: str | None = None, video: str | None = None) -> None:
    """Share a text update on LinkedIn, optionally with an image or video."""
    person_urn = f"urn:li:person:{cfg['person_id']}"

    if image and video:
        print(json.dumps({"error": "Cannot attach both image and video"}), file=sys.stderr)
        sys.exit(1)
    ...
    if image:
        ...
        asset_urn = upload_video(cfg, video)
        share_content = {
            "shareCommentary": {"text": text},
...
def main():
    ...
    p = sub.add_parser("post", help="Share a text update, optionally with an image or video")
    p.add_argument("text", help="Post content")
    p.add_argument("--image", help="Path to image file (jpg, png, gif)", default=None)
    p.add_argument("--video", help="Path to video file (mp4)", default=None)
    ...
    try:
        if args.command == "profile":
            ...
            cmd_post(cfg, args.text, image=args.image, video=args.video)
